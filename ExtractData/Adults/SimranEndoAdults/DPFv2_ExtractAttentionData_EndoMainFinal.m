...
Blocks=PF_EndoAttnParseFilesExp1(Observer);
num_blocks = size(Blocks,1);
FixBreaks_Combined = zeros(1,length(Blocks));

for i=1:size(Blocks,1)
    if Blocks(i,2)<10
    eval(sprintf('load(''data/%s/%g_stim0%g.mat'')',Observer,Blocks(i,1),Blocks(i,2)));
    Data{i}=getTaskParameters(myscreen,task);
    Contrast(i,:)=mean(stimulus.contrasts);
    %Contrast(i,:)=stimulus.contrasts;
    NumFixBreak(i)=sum(stimulus.FixationBreak);
    fixBreak{i}=stimulus.FixationBreak;
    added_trials = stimulus.trialend - length(stimulus.FixationBreak);
    extra_trials = zeros(1,added_trials);
    real_stimbreak = [stimulus.FixationBreak, extra_trials];
    centralfix_trials = find(real_stimbreak == 0);
    NumFixBreak(i)=(sum(stimulus.FixationBreak));
    fixBreak{i}=stimulus.FixationBreak;
    else
...
for i=1:size(Blocks,1)
    ...
    sum(fixIndex);
    temp1=[ temp1;Data{i}.randVars.targetOrientation(fixIndex)' Data{i}.response(fixIndex)' Data{i}.randVars.targetLocation(fixIndex)' Data{i}.randVars.CueCondition(fixIndex)' Data{i}.reactionTime(fixIndex)'];

    SubjectDataOriginal=[ SubjectDataOriginal;Data{i}.randVars.targetOrientation(fixIndex)' Data{i}.response(fixIndex)' Data{i}.randVars.targetLocation(fixIndex)' Data{i}.randVars.contrast(fixIndex)' Data{i}.randVars.CueCondition(fixIndex)' Data{i}.reactionTime(fixIndex)' Data{i}.blockNum(fixIndex)'];
...
SubjectDataOriginal.Properties.VariableNames = {'targetOrientation', 'Response', 'TargetLoc', 'Contrast','CueCondition', 'RT','Trial','rightwrong'};
...
%% 16 now make separate structs for valid and neutral
    ...
    indexValid =  SubjectDataOriginal.CueCondition == 1;
    SubjectDataValid = SubjectDataOriginal(indexValid,:);
    clear indexValid

    indexNeutral =  SubjectDataOriginal.CueCondition == 2;
...
cd /Volumes/purplab/EXPERIMENTS/1_Current_Experiments/Caroline/Caroline2/DPF_V2_(all)/DPFv2Scripts/ExtractDataAdults/SimranEndoAdults2
